...
if (isset($_SESSION["tip3Nombre"]))
    $tip3Nombre = $_SESSION["tip3Nombre"];
if (isset($_SESSION["tip3desc"]))
    $tip3desc = $_SESSION["tip3desc"];
if (isset($_SESSION["tip3img"]))
    $tip3img = $_SESSION["tip3img"];
if (isset($_SESSION["tpNumRad"]))
    $tpNumRad = $_SESSION["tpNumRad"];
if (isset($_SESSION["tpPerRad"]))
    $tpPerRad = $_SESSION["tpPerRad"];
if (isset($_SESSION["tpDescRad"]))
    $tpDescRad = $_SESSION["tpDescRad"];
if (isset($_SESSION["tip3Nombre"]))
    $tip3Nombre = $_SESSION["tip3Nombre"];
if (isset($_SESSION["ESTILOS_PATH"]))
...
<html>
    <head>
        <link rel="stylesheet" href="<?= $url_raiz . $ESTILOS_PATH ?>orfeo.css">
    ...
    */
    ...
    <body><form action=correspondencia.php method="get">
            ...
            $isql = "select a.*,b.depe_nomb from usuario a,dependencia b where a.depe_codi=b.depe_codi AND USUA_LOGIN ='$krd' ";
            $rs = $db->query($isql);

            $phpsession = session_name() . "=" . session_id();
            $usualoginuevo = $assoc == 0 ? trim($rs->fields["usua_login"]) : trim($rs->fields["USUA_LOGIN"]);
            echo "<font size=1 face=verdana>";
            // Valida Longin y contraseña encriptada con funcion md5()
            if ($usualoginuevo == trim($krd)) {
                ...
                if (trim($contraxx)) {
                    ...
                    $perrad = $assoc == 0 ? $rs->fields["perm_radi"] : $rs->fields["PERM_RADI"];

                    // Esta consulta selecciona las carpetas Basicas de DocuImage que son extraidas de la tabla Carp_Codi
                    $isqlConsulta = "select CARP_CODI,CARP_DESC from carpeta where mostrar_como_tipo=1 order by carp_codi ";
                    $rsConsulta = $db->query($isqlConsulta);
                    
                    //Adicionado as contador
                    // si el usuario tiene permiso de radicar el prog. muestra los iconos de radicaci�
                    include "$dir_raiz/menu/menuPrimero.php";
                    include "$dir_raiz/menu/radicacion.php";
                                    ...
                                    <tr>
                                        <td valign="top">
                                            <table width="150"  border="0" cellpadding="0" cellspacing="3" id="carpetasPerm">
                                                <?php
                                                while (!$rsConsulta->EOF) {

                                                    if (!isset($data))
                                                        $data = "NULL";

                                                    $numdata = $assoc == 0 ? trim($rsConsulta->fields["carp_codi"]) : trim($rsConsulta->fields["CARP_CODI"]);
...
//                                                    $db->conn->debug = true;
                                                    $descripcionCarpeta = $assoc == 0 ? $rsConsulta->fields["sgd_carp_descr"] : $rsConsulta->fields["SGD_CARP_DESCR"];
                                                    ( $descripcionCarpeta ) ? $data = $descripcionCarpeta : $data = $assoc == 0 ? trim($rsConsulta->fields["carp_desc"]) : trim($rsConsulta->fields["CARP_DESC"]);

                                                    if ($numdata == 11) {   // Se realiza la cuenta de radicados en Visto Bueno VoBo
                                                        $isqlCountRad = "select count(*) as CONTADOR from radicado where carp_per=0 and carp_codi=$numdata and radi_depe_actu='$dependencia' and radi_usua_actu=$codusuario and radi_nume_deri='0'";
                                                        $addadm = "&adm=1";
                                                    }
                                                    /***
                                                    Autor: Andrés Mosquera
                                                    Fecha: 2019-09-06
                                                    Info: Contador para la carpeta de reasignación masiva
                                                    ***/
                                                    elseif($numdata == 14) {
                                                        $isqlCountRad = "select count(*) as CONTADOR from radicado where carp_codi=$numdata and radi_depe_actu='$dependencia' and radi_usua_actu=$codusuario";
                                                    }
                                                    /***
                                                    Autor: Andrés Mosquera
                                                    Fecha: 2019-09-06
                                                    ...
                                                    ***/ 
                                                    ...
                                                    $flag = 0;
                                                    $rs1CountRad = $db->query($isqlCountRad);
                                                    $numerot = $rs1CountRad->fields["CONTADOR"];
                                                    if ($flag == 1)
                                                        echo "$isql";
                                                    ?>
                                                    <tr  valign="middle"><td width="125">
...
//                                                $db->conn->debug = true;
                    ...
                    <!--                                    <a onclick="cambioMenu(<?= $i ?>);verPersonales(<?= $i ?>);" href='#marcaPersonales";' class="menu_princ menu3"  alt="Despliegue Administración de Carpetas Personales" title="Despliegue de Carpetas Personales" aria-label="Despliegue Administración de Carpetas Personales" name="marcaPersonales">-->
                                            ...
                                            <table width="100%"  border="0" cellpadding="0" cellspacing="0" bgcolor="cacac9" id="carpersolanes">
                                                ...
                                                $isql = "select CODI_CARP,DESC_CARP,NOMB_CARP from carpeta_per where usua_codi=$codusuario and depe_codi='$dependencia' order by codi_carp asc ";
                                                $rsCODI_CARP = $db->query($isql);
                                                $conteoRegistros = 0;

                                                while (!$rsCODI_CARP->EOF) {
                                                    if ($data == "")$data = "NULL";
                                                    $data = trim($rsCODI_CARP->fields["NOMB_CARP"]);
                                                    $numdata = trim($rsCODI_CARP->fields["CODI_CARP"]);
                                                    $detalle = trim($rsCODI_CARP->fields["DESC_CARP"]);
                                                    ...
                                                    if($conteoRegistros < 9){                                                        
                                                        ?>
                                                        <tr>
                                                            <td height="18"><a href="cuerpo.php?<?= $phpsession ?>&fechah=<?=$fechah?>&tipo_carp=1&carpeta=<?=$numdata?>&nomcarpeta=<?=$data?> (Personal) " alt="Carpeta Personal <?= $detalle ?>" title="Carpeta Personal <?= $detalle ?>" aria-label="Carpeta Personal <?= $detalle ?>" class="itemsSubmenu3" target="mainFrame"><? echo "$data ($numerot)";?></a> </td>
                                                        </tr>
                                                        <?php
                                                    }
                                                    else{
                                                        ?>
                                                        <tr>
                                                            <td height="18"><a href="cuerpo.php?<?= $phpsession ?>&fechah=<?=$fechah?>&tipo_carp=1&carpeta=<?=$numdata?>&nomcarpeta=<?=$data?> (Personal) " alt="Carpeta Personal <?= $detalle ?>" title="Carpeta Personal <?= $detalle ?>" aria-label="Carpeta Personal <?= $detalle ?>" class="itemsSubmenu3parte" target="mainFrame"><? echo "$data ($numerot)";?></a> </td>
                                                        </tr>
                                                        <?php 
                                                    }

                                                    if($conteoRegistros == 8){                                                        
                                                        ?>
                                                        <tr>
                                                            <td height="18"><a href="cuerpo.php?<?= $phpsession ?>&fechah=<?=$fechah?>&tipo_carp=1&carpeta=<?=$numdata?>&nomcarpeta=<?=$data?> (Personal) " alt="Carpeta Personal <?= $detalle ?>" title="Carpeta Personal <?= $detalle ?>" aria-label="Carpeta Personal <?= $detalle ?>" class="itemsSubmenu3segunda" target="mainFrame"><? echo "$data ($numerot)";?></a> </td>
                                                        </tr>
                                                        <?php
                                                    }
                                                    
                                                    $conteoRegistros++;
                                                    $rsCODI_CARP->MoveNext();                                                    
                                                }
                                                ?>
                                            </table>
                                        </td>
