...
$ESTILOS_PATH2 =$_SESSION['ESTILOS_PATH2'];
	...
	function crearProcesoExpediente($numExpediente,$radicado,$depe_codi,$usua_codi,$usua_doc,$usuaDocExp,$codiSRD,$codiSBRD,$fechaExp,$proc){
		...
		/*By skina
		* Si ya tengo un expediente, quiero ponerle un proceso
		*/
		//Busco la etapa inicial del proceso que quiero
		...
		$record["SGD_HFLD_OBSERVA"] = "'Adicion del Proceso $flujo_nombre'";
		$record["SGD_FARS_CODIGO"] = "0";
		$record["SGD_HFLD_AUTOMATICO"] = 1;
	
	if(!$fechaExp) $fechaExp = $fecha_hoy;
	$sqlFechaHoy=$db->conn->OffsetDate(0,$db->conn->sysTimeStamp);
		//by skina $query="insert into SGD_SEXP_SECEXPEDIENTES(SGD_EXP_NUMERO   ,SGD_SEXP_FECH      ,DEPE_CODI   ,USUA_DOC   ,SGD_FEXP_CODIGO,SGD_SRD_CODIGO,SGD_SBRD_CODIGO,SGD_SEXP_SECUENCIA, SGD_SEXP_ANO,USUA_DOC_RESPONSABLE,SGD_PEXP_CODIGO) VALUES ('$numExpediente',". $sqlFechaHoy ." ,'$depe_codi','$usua_doc',$codEtapa,$codiSRD,$codiSBRD,$secExp ,$anoExp,$usuaDocExp,$proc)";
		$query="update SGD_SEXP_SECEXPEDIENTES set SGD_SEXP_FECH =". $sqlFechaHoy ." ,DEPE_CODI= '$depe_codi' ,
					USUA_DOC='$usua_doc' ,SGD_FEXP_CODIGO=$codEtapa ,SGD_SRD_CODIGO=$codiSRD ,SGD_SBRD_CODIGO=$codiSBRD,
					SGD_SEXP_SECUENCIA=$secExp, SGD_SEXP_ANO=$anoExp,USUA_DOC_RESPONSABLE=$usuaDocExp ,SGD_PEXP_CODIGO=$proc
...
<form method="post" action="<?=$encabezadol?>" name="TipoDocu" onsubmit="return validar();">
    <center>
        <div id="titulo" style="width: 70%;" align="center">Adicionar proceso al expediente</div>
    </center>
 <table style="width:70%;" border="1" cellspacing="1" cellpadding="0" align="center" class="borde_tab">
<!--   <tr align="center" class="titulos2">
      <td height="15" class="titulos2" colspan="2">ADICIONAR PROCESO AL EXPEDIENTE</td>
    </tr>-->
<tr >
...
<?php
	...
	print $rsD->GetMenu2("codserie", $codserie, "0:-- Seleccione --", false,"","onChange='submit()' class='select' id='codserie' aria-label='Listado de series disponibles para asignar al expediente'" );
 ?>
	</td>
	</tr>
	<tr>
	...
	include "$dir_raiz/include/tx/ComentarioTx.php";
	print $rsSub->GetMenu2("tsub", $tsub, "0:-- Seleccione --", false,"","onChange='submit()' class='select' id='tsub' aria-label='Listado de subseries de la serie para aplicar al expediente'" );
	if(!$codiSRD)
	{
		$codiSRD = $codserie;
		$codiSBRD = $tsub;
...
?>
	...
	<tr>
        ...
        <td class="listado2" colspan="2" >
          <?
            ...
            print $rs->GetMenu2("codProc", $codProc, "0:-- Seleccione --", false,"","onChange='submit()' class='select' id='codProc' aria-label='Listado con los procesos disponibles a asignar' " );
	...
	<tr>
	<td class="titulos5" colspan="2">Se dispone a  Crear un Proceso en el Expediente N&middot;</td>
	</tr>
	<tr>
...
<TD class=titulos5 nowrap>
	...
	<td class=listado2>
<?
	$queryUs = "select usua_nomb, usua_doc from usuario 
			where depe_codi='$dependencia' AND USUA_ESTA='1'
			order by usua_nomb";
	$rsUs = $db->conn->Execute($queryUs);
	print $rsUs->GetMenu2("usuaDocExp", "$usuaDocExp", "0:-- Seleccione --", false,""," class='select' onChange='submit()' aria-label='Lista con usuarios de la dependencia para asignar como responsable del proceso'");
...
?>
	...
	<?php  if($tsub && $codserie && $usuaDocExp) {?>
			<input name="CrearProceso" type="submit" id='crearProceso' class="botones_funcion" value="Crear Proceso" aria-label="Boton crear proceso en el expediente" />
	<?php } ?>
	</center></TD>
	<td width="33%" class="listado2" height="25">
	<center><input name="cerrar" type="button" class="botones_mediano" id="envia22" onClick="opener.regresar(); window.close();" value=" Cerrar " aria-label="Boton cerrar ventana"></center></TD>
	</tr>
</table>
</form>
</body>
