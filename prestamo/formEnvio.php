...
if ($pageAnt == $sFileName && $ordenar == 0) {
    $query = "count(PRES_ID) as TOTAL from PRESTAMO where PRES_ID in ($setFiltroSelect) and USUA_LOGIN_ACTU='" . $usua_codi . "'";
    $rs = $db->conn->query($query);
    if ($rs && !$rs->EOF) {
...
if ($opcionMenu == 3) {  //cancelar
    ...
    if ($pageAnt == $sFileName) {
        ...
        // Validaci�n de la contraseña
        //$flds_CONTRASENA=strip(get_param("s_CONTRASENA"));		 
        $flds_CONTRASENA = $_POST['s_CONTRASENA'];
...
if ($enviar == 1) {  // Llama la p�gina que hace el procesamiento
    echo " .. ";
    echo "<form action='../solicitar/Reservar.php?<?=$encabezado?>' method='post' name='go'> </form><br>";
    echo "<script>document.go.submit();</script>";
}

//   $queryss="select PARAM_CODI, PARAM_VALOR from SGD_PARAMETRO where PARAM_NOMB='PRESTAMO_ESTADO' and PARAM_CODI in (2,5)"; 
//      $rsListado = $db->conn->query($queryss);
//      $db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
//      echo '------------------- '.$queryss;
//      echo '<pre>';
//      print_r($rsListado->fields);
//      echo '</pre>';
// Build SQL
$sSQLsele = " and P.PRES_ID in (" . $setFiltroSelect . ") ";
include $dir_raiz . "/include/query/prestamo/builtSQL1.inc";
include $dir_raiz . "/include/query/prestamo/builtSQL3.inc";
...
<body bgcolor="#FFFFFF" topmargin="0">
    <table border=0 width=100% cellpadding="0" align="center" cellspacing="0">
        <tr>
            <td width=100%><br>
                <form action='/solicitar/Reservar.php?<?= $encabezado ?>' method=post name="rta" >
                    ...
                    <table width="95%" border="0" cellpadding="0" cellspacing="5" align="center" class="borde_tab">
                        <tr>
                            <TD width=30% class="titulos4">USUARIO:<br><br><?= $_SESSION['usua_nomb'] ?><br></TD>
                            <TD width='30%' class="titulos4">DEPENDENCIA:<br><br><?= $_SESSION['depe_nomb'] ?><br></TD>
                            <TD width="30%" class="titulos4"><?= $titCaj ?><BR></td>
                            <td width='5' class="titulos4"><input type=button value=Realizar onclick="okTx();" name=enviardoc align=bottom class=botones id=REALIZAR></td>
                        </tr>
                        <tr align="center">
                            <td colspan="4" class="celdaGris" align=center><br><center>
                            <table width="100%"  border=0 align="center" bgcolor="White">
                                <tr bgcolor="White">
                                    <td width="100"><center><img src="<?= $url_raiz ?>/iconos/tuxTx.gif" alt="Tux Transaccion" title="Tux Transaccion"></center></td>
                                <td align="left"><textarea name=observa cols=70 rows=3 class=ecajasfecha><?= $observa ?></textarea></TD>
                                </tr>  			             
<?php if ($opcionMenu == 1) {  // Prestamo ?>
                                    <tr bgcolor="White">
    ...
    $queryss = "select PARAM_CODI, PARAM_VALOR from SGD_PARAMETRO where PARAM_NOMB='PRESTAMO_ESTADO' and PARAM_CODI in (2,5)";
    $rsListado = $db->conn->query($queryss);
    while ($rsListado && !$rsListado->EOF) {
        $idEstado = isset($rsListado->fields["PARAM_CODI"]) ? $rsListado->fields["PARAM_CODI"] : $rsListado->fields[0];
        $txtEstado = isset($rsListado->fields["PARAM_VALOR"]) ? $rsListado->fields["PARAM_VALOR"] : $rsListado->fields[1];
        $x = "";
        if ($flds_PRES_ESTADO == $idEstado) {
            $x = " selected ";
        }
                            ...
                            <table width="100%"  border=0 align="center" bgcolor="White">
                                <TR bgcolor="White">
                                    <TD width="100%" align="center">					 
                                        <table align="center" border=1 cellpadding=0 cellspacing=2 class='borde_tab' width="100%">	   
                                            <?PHP
   ...
   if(numCaracteres>=6){
        ...
        document.rta.action = '<?= $sFileName ?>?<?= $encabezado ?>';
            document.rta.submit();
                    ...
                    alert("Atención: El numero de Caracteres mínimo en la Observacion es de 6. (Digito :" + numCaracteres + ")");
            return 0;
      }	 	     
      }
