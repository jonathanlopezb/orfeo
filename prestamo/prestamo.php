...
*  Modificado: 1/3/2006  IIAC  Basado en pedido.php. Facilita la b�squeda de los registros de pr�stamo.       *
* ******************************************************************************* ****************************/
//===============================
// prestamo begin
//===============================
...
<html>
    <head>
        ...
        <script src="<?= $url_raiz ?>/js/popcalendar.js"></script>
...
//                $haceXDias = strtotime("-" . $x . " day");
    ...
    <form method="post" action="prestamo.php" name="busqueda">
        ...
        <script language="javascript">
            setRutaRaiz ('..'); // Para el calendario
        </script>
        <br>
        <center>
            <table border=1 width="100%" cellpadding=0 cellspacing=2 class='borde_tab'>	  
                <tr>
                <div id="titulo" align="center" style="padding: 1.5%;">
                    <?//=ucfirst(strtolower($sFormTitle[$opcionMenu])); ?>
                    &nbsp;<?= $sFormTitle[$opcionMenu]; ?>&nbsp;
                </div>
                </tr>  
                <tr id="b0" style="display:<?= $tipoBusqueda[$opcionMenu][0]; ?>">
                    <td class="titulos2"><p align="left"><label for="s_RADI_NUME_RADI">Radicado</label></p></td>
                    <td class="listado2"><input type="text" id="s_RADI_NUME_RADI" name="s_RADI_NUME_RADI" maxlength="20" value="<?= $flds_RADI_NUME_RADI; ?>" size="25" class="tex_area" title="Campo para ingresar el número o parte del número del radicado"></td>
                    <td class="titulos2"><p align="left"><label for="s_USUA_LOGIN">Login de Usuario</label></p></td>
                    <td class="listado2"><input type="text" id="s_USUA_LOGIN" name="s_USUA_LOGIN" maxlength="15" value="<?= $flds_USUA_LOGIN; ?>" size="25" class="tex_area" title="Campo para ingresar el Login o nombre de sesión del usuario"></td>
                </tr>	  
...
<!--                    <td class="titulos2"><p align="left"><label for="s_USUA_LOGIN">Login de Usuario</label></p></td>
                ...
                <tr id="b2" style="display:<?= $tipoBusqueda[$opcionMenu][2]; ?>">
                    <td class="titulos2"><p align="left"><label for="s_DEPE_NOMB">Dependencia</label></p></td>
                    <td class="listado2"><select id="s_DEPE_NOMB" name="s_DEPE_NOMB" class="select" onChange=" document.busqueda.s_sql.value = 'no'; document.busqueda.submit();" title="Listado con todas las dependencias existentes, al cambiar de dependencia se actualiza el listado de usuarios">
                            <option value="">- Todas las dependencias -</option>
                            <?php
                            $lookup_s = db_fill_array("select DEPE_CODI, ".$db->conn->Concat( "DEPE_CODI", "' - '", "DEPE_NOMB" )." as DEPE_NOMB from DEPENDENCIA order by 2");
                        ...
                        <td class="titulos2"><p align="left"><label for="s_USUA_NOMB">Usuario</label></p></td>
                    <td class="listado2"><select name="s_USUA_NOMB" class=select id="s_USUA_NOMB" title="Listado con todos los usuarios registrados en el sistema">
...
<!--                    <td class="titulos2"><p align="left"><label for="s_USUA_NOMB">Usuario</label></p></td>
                ...
                <tr id="b4" style="display:<?= $tipoBusqueda[$opcionMenu][4]; ?>">
                    <td class="titulos2"><p align="left"><label for="s_PRES_REQUERIMIENTO">Requerimiento</label></p></td>	 
                    <td class="listado2"><select id="s_PRES_REQUERIMIENTO" name="s_PRES_REQUERIMIENTO" class=select title="Listado con tipos de requerimientos">
                        ...
                        <td class="titulos2"><p align="left"><label for="s_PRES_ESTADO">Estado</label></p></td>	 
                    <td class="listado2"><select name="s_PRES_ESTADO" class=select id="s_PRES_ESTADO" title="Listado con los posibles estados en los que un documento puede estar">
...
<!--                <tr id="b5" style="display:<?= $tipoBusqueda[$opcionMenu][5]; ?>">
                ...
                <tr id="b6" >
                    <td class="titulos2"><p align="left"><label for="fechaInicial">Fecha inicial</label><br>&nbsp;&nbsp;(aaaa-mm-dd)</p></td>	 
                    <td class="listado2">
                        <script language="javascript">
                    ...
                    <td class="titulos2"><p align="left"><label for="fechaFinal">Fecha final</label><br>&nbsp;&nbsp;(aaaa-mm-dd)</p></td>
                    <td class="listado2"><script language="javascript">
...
<!--                <tr id="b7" style="display:<?= $tipoBusqueda[$opcionMenu][7]; ?>">
                ...
                <tr id="b8" style="display:<?= $tipoBusqueda[$opcionMenu][8]; ?>">
                    <td class="titulos2"><p align="left">Hora límite</p></td>
                    <td class="listado2"><label for="s_hora_limite">hora<select name="s_hora_limite" class=select id="s_hora_limite" title="Listado con las 12 horas del día">
                                <?php
                                for ($i = 1; $i <= 12; $i++) {
                                    if ($i <= 9) {
                                        $h = "0" . $i;
                            ...
                            <label for="s_minuto_limite">minutos</label>
                            <select id="s_minuto_limite" name="s_minuto_limite" class=select title="Listado con los 60 minutos de la hora">
                                <?php
                                for ($i = 0; $i <= 59; $i++) {
                                    if ($i <= 9) {
                                        $h = "0" . $i;
                            ...
                            <label for="s_meridiano">meridiano</label>
                            <select name="s_meridiano" id="s_meridiano" class=select title="listado con los tipos de meridiano, a eme o pm">
                                <?php if ($v_meridiano == "AM") { ?>								 
...
//===============================
                            function Pedidos_Show() {
                                ...
                                if (strlen($pageAnt) == 0) {
                                    // Build SQL  		 		
                                    include_once $dir_raiz . "/include/query/prestamo/builtSQL1.inc";
                                    include_once $dir_raiz . "/include/query/prestamo/builtSQL2.inc";
                                    include_once $dir_raiz . "/include/query/prestamo/builtSQL3.inc";
...
//                                    echo '++++++++++++++++ '.$sSQL;
                                    ...
                                    if (!$rs || $rs->EOF) {
                                        ?>
                                        <p align="center" class="titulosError2">No se encontraron solicitudes de prestamos pendientes </p>
                                        <?php
                                        return;
                                    }
                                    ...
                                    <form method="post" action="prestamo.php" name="rta">
                                        ...
                                        <table border=1 cellpadding=0 cellspacing=2 class='borde_tab' width="100%">
                                            <tr>
                                            <div id="titulo" align="center" >
                                                <?//=ucfirst(strtolower($sFormTitle[$opcionMenu])); ?>
                                                <?= $tituloRespuesta[$opcionMenu] ?>
                                            </div>
                                            </tr>  
                            <!--                <tr>
                                            ...
                                            if ($iPage > 1) {
                                                do {
                                                    $new = $assoc == 1 ? $rs->fields["prestamo_id"] : $rs->fields["PRESTAMO_ID"];
                                                    if ($new != $ant) {
                                                        $iCounter++;
                                                        $ant = $new;
                                                    }
                                            ...
                                            while ($rs && !$rs->EOF && $y < $iRecordsPerPage) {
                                                ...
                                                if ($antfldPRESTAMO_ID != $fldPRESTAMO_ID) { //Une en un solo campo los expedientes
                                                    ...
                                                    if ($fldARCH == 'SI') {
                                                        ...
                                                        $antfldARCH .= "<a href='" . $dir_raiz . "/expediente/datos_expediente.php?" . $encabARCH . "&num_expediente=" . tourl($fldEXP) . "&nurad=" . tourl($antfldRADICADO) . "' class='vinculos'>" . $fldARCH . "</a>";
                                                    } else {
                                                        $antfldARCH .= $fldARCH;
                                                    }
                                                }
        ...
        function regresar() {
			             ...
			             <font class=\"ColumnFONT vinculosCabezote \" title=\"Ver la p&aacute;gina " . $iPageCount . "\">" . $iPageCount . "</font></a>&nbsp;";
                                                    } else {
                                                        break;
                                                    }
                                                }
                         ...
                         &FormSiguiente=1&\"><font class=\"ColumnFONT\" title=\"Ver la &uacute;ltima p&aacute;gina\">>|</font></a>";
                                                ...
                                                <tr class="titulos2" align="center">
                                                    <td colspan="<?= ($numCol + 1); ?>"><center><?= $sPages ?><br>P&aacute;gina <?= $iPage ?>/<?= $iHasPages ?><br>
                                                    Total de Registros: <?= $iCounter ?></center></td>
                                                </tr>
                                                <?php
