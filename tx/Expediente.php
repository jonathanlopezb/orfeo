...
//        ECHO "<hr>**************** prueba......*************<hr>";
    ...
    function consulta_exp($radicado) {
        ...
        $query = "select SGD_EXP_NUMERO,$radi_nume_radi RADI_NUME_RADI  from SGD_EXP_EXPEDIENTE a where RADI_NUME_RADI = $radicado";
    ...
    function insertar_expediente($num_expediente, $radicado, $depe_codi, $usua_codi, $usua_doc) {
        $estado_expediente = 0;
        $creadoOld = 0;
        $query = "select * from SGD_SEXP_SECEXPEDIENTES WHERE SGD_EXP_NUMERO='$num_expediente' ";
        $rs = $this->db->conn->query($query);
        if (!$rs->fields["SGD_EXP_NUMERO"]) {
            $this->db->conn->query($query);
            $query = "select SGD_EXP_NUMERO, SGD_EXP_FECH from SGD_EXP_EXPEDIENTE WHERE SGD_EXP_NUMERO='$num_expediente' order by SGD_EXP_FECH DESC";
