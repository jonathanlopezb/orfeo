...
$db->conn->SetFetchMode(ADODB_FETCH_ASSOC); // Esto permite que se muestre los nombres de las columnas y no numeros
//$db->conn->debug = true;
...
if ($checkValue) {
    ...
    while (list($recordid, $tmp) = each($checkValue)) {
        ...
        switch ($codTx) {
            ...
            case 9:   //edd3 
                $isqlTRDA = "select  RADI_NUME_SALIDA from anexos where ANEX_RADI_NUME = '$record_id'";
            ...
            case 13: {
                    ...
                    if ($codTx == 16 or $codTx == 65 or $codTx == 13) {
                        ...
                        $radiNumero = $assoc == 0 ? $rsTRDP->fields["radi_nume_radi"] : $rsTRDP->fields["RADI_NUME_RADI"];
        ...
        if ($mensaje_errorEXP || $mensaje_errorDig || $mensaje_error) {
            ...
            <table border=0 width=100% cellpadding="0" cellspacing="0">
                <tr>
                    <td width=100%>
                        <br>
                        <form action='realizarTx.php?<?= $encabezado ?>' method='GET'  name="realizarTx" id="realizarTx" >
                            ...
                            <input type="hidden" value="" name="numExpHidden" id="numExpHidden">
                            <center>
                                <table width="98%" border="0" cellpadding="0" cellspacing="5" class="borde_tabReducido">
                                    <TR class="titulos4 tdTitulo">
                                        ...
                                        <td rowspan="2" >
                                            ...
                                            $var_usua_codi = "cast(u.usua_codi AS character(20))";
                                            ...
                                            switch ($codTx) {
                                                ...
                                                case 8: $usDefault = 1;
//                                                    $driver == 'postgres' ? $var_usua_codi = "cast(u.usua_codi AS character(20))" : "CONVERT(varchar(20),u.usua_codi)";
...
//                                                        error_log('paso por aqui linea 589 formEnvio.php '.$dependencia);
                                                    ...
                                                    /* } */
//                                                    $var_usua_codi = "cast(u.usua_codi AS character(20))";
                                                    $cad = $db->conn->Concat("u.depe_codi", "'-'", $var_usua_codi);
                                                    $sql = "select u.USUA_NOMB , $cad as USUA_COD ,u.DEPE_CODI,u.USUA_CODI from usuario u where u.USUA_ESTA='1' $whereReasignar $whereDep $usuario_publico ORDER BY USUA_NOMB";
                                                    ...
                                                    <select name=usCodSelect class=select aria-label="Listado de usuarios disponibles de la dependencia para reasignar, Seleccione uno.">
                                                        ...
                                                        while (!$rs->EOF) {
                                                            if ($rs->fields["DEPE_CODI"]) {
                                                                ...
                                                                $usuario_nomb = $rs->fields["USUA_NOMB"];
                                                                $usuario_cod = $rs->fields["USUA_COD"];
                                                                $usuario_codi = $rs->fields["USUA_CODI"];
                                                            } else {
                                                                $depe_codi = $rs->fields[2];
                                                                $usuario_nomb = $rs->fields[0];
                                                                $usuario_cod = $rs->fields[1];
                                                                $usuario_codi = $rs->fields[3];
                                                            }

                                                            $depCodiP = $depe_codi;
                                                            $usuNombP = $usuario_nomb;
                                                            $usuCodiP = $usuario_cod;
                                                            $usuCodi = $usuario_codi;
...
//                                                    error_log('******************* '.$sql);
                                                    $carpetaNombre = $assoc == 0 ? $rs->fields['carp_desc'] : $rs->fields['CARP_DESC'];
				...
				";
                                    ...
                                    <center>
                                        <table width="500"  border=0 align="center" bgcolor="White">
                                            ...
                                            <center>
                                                <img src="<?= $url_raiz ?>/iconos/tuxTx.gif" alt="Imagen Logo Transacción Orfeo" title="Imagen Logo Transaccion Orfeo">
                                            </center>
                                        </td><TD align="left">
                                        <span class="etextomenu">
                                        </span>
                                        <textarea name=observa id=observa cols=70 rows=3 aria-label='Caja de texto para describir asunto o observación para la transacción actual' class=ecajasfecha title='Caja de texto para describir asunto o observación para la transacción actual'></textarea>
                                    <td width='5' class="grisCCCCCC">
                                        <input type=button  value=Realizar 
                                               onClick="<?php if ($codTx == 8) { ?> if (document.getElementById('usCodSelect').value == ''){ alert('Debe seleccionar al menos un funcionario.'); return false; }<?php } ?> <?php if ($codTx == 9) { ?>
                if (document.realizarTx.usCodSelect.value == '-1')
                ...
                return false;
                        ...
                        switch ($codTx) {
                            ...
                            case 61: {
                                    echo '<script>window.open("../radicacion/tipificar_documento_multiple.php?whereFiltro=' . $whereFiltro . '&whereFiltro2=' . $whereFiltro2 . '&krd=' . $krd . '&multiple=1", "Tipificacion de documentos", "height=500,width=750,scrollbars=yes"); window.focus();</script>';
