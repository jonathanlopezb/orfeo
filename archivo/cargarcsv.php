	...
	if ($my_upload->upload($newFile)) {
		...
		if($arch==1){
			...
			{  
				...
				sgd_archivo_zona,sgd_archivo_carro,sgd_archivo_cara,sgd_archivo_estante,sgd_archivo_entrepano,sgd_archivo_caja,sgd_archivo_rad,sgd_archivo_srd,sgd_archivo_sbrd,depe_codi,sgd_archivo_usua,sgd_archivo_mata)
				values ('$sec','$depe','$tipo','$orden','$deman','$demant','$ano','$unidocu',$timei,$timef,'$anexo','$folios','$zona','$carro','$cara','$estante','$entrepa','$caja','$expe','$srd','$sbrd','$depeU','$krd','$mata')";
				//$db->conn->debug=true;
				$rs3=$db->query($sqlm);
				if($rs3->EOF)$p++;
				}
