        ...
        <form name="relacionTiposAlmac" action="<?= $encabezadol ?>" method="POST" >
            ...
            if ($grabar) {
                ...
                if ($tipoAlmacPadre == $codp) {
                    ...
                    $sql = "insert into sgd_eit_items (sgd_eit_codigo,sgd_eit_cod_padre,sgd_eit_nombre,sgd_eit_sigla) values ( " . $db->conn->nextId('SEC_EDIFICIO',$driver) . ",'$tipoAlmacPadre','$nom','$sig')";
                    ...
                    for ($i = 1; $i <= $cantidad; $i++) {
                        ...
                        $sql = "insert into sgd_eit_items (sgd_eit_codigo,sgd_eit_cod_padre,sgd_eit_nombre,sgd_eit_sigla) values ( " . $db->conn->nextId('SEC_EDIFICIO',$driver) . ",'$tipoAlmacPadre','$nomc','$sigc')";
                        //$db->conn->debug=true;
                        $rs = $db->conn->Execute($sql);
                    }
                }
...
//                            $db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
                            ...
                            $c = 0;
                            $cp = 0;
                            $conD = $db->conn->Concat("convert(char(18), sgd_eit_codigo, 0)", "'-'", "sgd_eit_nombre");
                            //skina
