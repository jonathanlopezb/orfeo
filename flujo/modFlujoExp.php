...
//Modificado Skina 17-02-09 Se agrego la linea de conexion a la DB
...
if($grabarFlujo)
{
  ...
  if(!$ddca_causal) 
	...
	$objFlujo = new Flujo($db, $texp,$usua_doc);
	list ($estadoNuevo, $aristaActual) = preg_split('-', $flujo);
	$expEstadoActual = $objFlujo->actualNodoExpediente($numeroExpediente);
	$observa .= " ($flujo_nombre)";
	$estadoModificado = $objFlujo->cambioNodoExpediente($numeroExpediente,$numRad,$estadoNuevo,$aristaActual,0,"Cambio Manual. $observa ($flujo_nombre)",$_GET['texp']);
