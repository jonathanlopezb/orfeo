        ...
        <meta http-equiv="X-UA-Compatible" content="IE=edge"><!-- CSS -->
            ...
            <script>
                ...
                function validarForm(quejas) {
                    ...
                    /* -------------- PERSONA NATURAL -----------------------*/
                    if (formulario.nom_ciu.value.length == 0 && tipoSolicitante == 1) { //comprueba que no esté vacío
                        formulario.nom_ciu.focus();
                        alert('Escriba Nombre Solicitante');
                        return false; //devolvemos el foco
                    }

                    if (formulario.apll1_ciu.value.length == 0 && tipoSolicitante == 1) { //comprueba que no esté vacío
                        formulario.apll1_ciu.focus();
                        alert('Escriba Primer Apellido');
                        return false; //devolvemos el foco
                    }
                    /*----------------- PERSONA JURIDICA ---------------------*/
                    if (formulario.raz_social.value.length == 0 && tipoSolicitante == 2) { //comprueba que no esté vacío
                        formulario.raz_social.focus();
                        alert('Escriba Razón Social');
                        return false; //devolvemos el foco
                    }
                    if (formulario.rep_legal.value.length == 0 && tipoSolicitante == 2) { //comprueba que no esté vacío
                        formulario.rep_legal.focus();
                        alert('Escriba Rep. Legal');
                        return false; //devolvemos el foco
                    }
                    /*--------------------------------------------------------------------------------------------*/
                ...
                function validarCaptcha() {
                    var cadena = btoa(document.getElementById('campo_captcha').value);
                    var str1 = document.getElementById('codecaptcha').value;
                    result = cadena.localeCompare(str1);
                    if (result != 0) {
                        $('#campo_captcha').css('border-color', 'red');
                        $('#saveForm').attr('disabled', 'true');
                    } else {
                        $('#campo_captcha').css('border-color', '');
                        $('#saveForm').remove('disabled');
                    }
                }

                function reloadImg(imagen) {
                    location.reload();
                }
            </script>
    </head>
    <body id="public">
        ...
        <div id="container">
            <form id="contactoOrfeo" class="wufoo topLabel" enctype="multipart/form-data" method="post" action="formulariotx.php" name="quejas" onsubmit="return validarForm(this);">
                <div class="info">
                    <center> <img src="../logoEntidad.png"  height="200px" align="center"/></center>
                    <p><h3>PQRS - Información</h3>
                        &nbsp </br>Objetivo del Sistema: Establecer las actividades para recibir, direccionar y hacer seguimiento al trámite de las Peticiones, Quejas, Reclamos, Sugerencias y Denuncias formuladas por los usuarios de los servicios que presta la Universidad de Córdoba, con el fin de mejorar la eficacia, eficiencia y efectividad de la respuesta a estas solicitudes.
                        </br></br>
                        ...
                        Si usted desea registrar su petición de forma anonima, por favor ingrese el número de identificación 111111111 y un correo valido.
                    </p>
                </div>
                <fieldset>
                    ...
                    <div id="solNat">
                        ...
                        <div id="li_apellido2">
                            <label for="apll2_ciu" id="lbl_apellido2" class="desc">Segundo Apellido: </label>
                            <div>
                ...
                <fieldset>
                    ...
                    <div id="datosSolicitud">
                        <div id="li_tipoSolicitud">
                            ...
                            <select id="tipoSolicitud" name="tipoSolicitud" tabindex="1" class="field select small" title="Lista desplegable de los tipos de solicitud disponibles para su elección">
                                <option value="0" selected="selected">Seleccione Tipo de solicitud</option>
                                <?= $tipo; ?>
                            </select>
                        </div>        
                        <div id="foli4">
                            <label class="desc" id="title4" for="label7">Referente al Radicado No.</label>
                            <input id="label7" name="radicado" type="text"  class="field text small" value="" maxlength="16"
                    ...
                    <div id="datosSolicitud">
                        <div id="li_tipoSolicitud">
                            <label class="desc" id="title_grupoInteres" for="grupoInteres">Tipos de usuarios<font color="#FF0000">*</font></label>
                            <select id="grupoInteres" style="width: 320%;" name="grupoInteres" tabindex="1" class="field select small" title="Lista desplegable de los tipos de usuarios o grupos de interés disponibles para su elección">
                                <option value="0" selected="selected">Seleccione</option>
                                <?= $tipoUsuariosGrupos; ?>
                            </select>
                        </div>
                    ...
                    <div>
                        <label class="desc" id="lbl_adjuntos" for="campo_adjuntos">Adjuntos(Máximo 5MB por archivo, 20MB en total)</label>
                        <input type="hidden" name="MAX_FILE_SIZE" value="<?php echo $max_file_size; ?>" /> 
                        <!--<input class="field file large" id="campo_adjuntos" name="userfile" type="file" onChange="addInputFile();" />-->
                        <input class="field file large" id="campo_adjuntos" name="userfile[]" multiple="multiple" type="file" onChange="addInputFile();" />
                    </div>
                </fieldset>
                <div id="li_imagenVerificacion idcaptcha"><label class="desc" id="lbl_captcha" for="campo_captcha">Imagen de
                    ...
                    <div>
                        <input id="campo_captcha" name="captcha" type="text"
                                class="field text small" value="" maxlength="7" tabindex="20"
                                onkeypress="return alpha(event, numbers + letters)"
                                onchange="return validarCaptcha()"
                                alt="Digite las letras y n&uacute;meros de la im&aacute;gen" /> &nbsp;
                        <!--<input type="text" name="dato" value="0" id="dato"/>-->
                        <p><?php
                            echo '<img id="imgcaptcha" src="' . $_SESSION['captcha_formulario']['image_src'] . '" alt="CAPTCHA" width="17%" style="text-align: center;"/>';
                            echo '<a href="#" onClick="return reloadImg(\'imgcaptcha\');"><img src="indice.png" width="4%" style=" float: right;margin-right: 79%;margin-top: 16px;"/><a>'
                            ?></p>
                        <input type="hidden" name="idFormulario" value="<?= $_SESSION["idFormulario"] ?>" />
                        <input type="hidden" name="codecaptcha" id="codecaptcha" value="<?= base64_encode($_SESSION['captcha_formulario']['code']) ?>" />
                    </div>
                </div>
