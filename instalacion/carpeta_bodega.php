...
if($_GET["anoCrear"]) $anoCrear = date('Y');
...
// Directorio firmasqr
// Verifica si no existe el directorio firmasqr
if ( ! is_dir( $ruta_raiz.'/bodega/firmasqr' ) )
{
    // Crea un directorio llamado firmasqr
    if ( mkdir ( $ruta_raiz.'/bodega/firmasqr', 0777 ) )
    {
        print "Directorio ".$ruta_raiz.'/bodega/firmasqr'." creado.<br>";
    }
    else
    {
        print "No se pudo crear el directorio ".$ruta_raiz.'/bodega/firmasqr.<br>';
    }
}
...
function creaDirDepe( $ruta_raiz ,$anoCrear)
{
    ...
    // Directorio Transfencias Documentales
    // Verifica si no existe el directorio transDocumentales
    if ( ! is_dir( $ruta_raiz.'/bodega/'.$anoCrear.'/transDocumentales' ) )
    {
        // Crea un directorio llamado transDocumentales
        if ( mkdir ( $ruta_raiz.'/bodega/'.$anoCrear.'/transDocumentales', 0777 ) )
        {
            print "Directorio ".$ruta_raiz.'/bodega/'.$anoCrear.'/transDocumentales'." creado.<br>";
        }
        else
        {
            print "No se pudo crear el directorio ".$ruta_raiz.'/bodega/'.$anoCrear.'/transDocumentales.<br>';
        }
    }
