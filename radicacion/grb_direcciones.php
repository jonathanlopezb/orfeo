...
if (!is_null($muni_us3)) {
    ...
    switch (count($muni_tmp3)) {
        case 4: {
                ...
                //$conexion->conn->nextId("sec_dir_direcciones", $conexion->driver);
            }break;
        case 3: {
                $idcont3 = $tmp_idcont;
                $idpais3 = $muni_tmp3[0];
...
$newId = false;
if (!$modificar) {
    $nextval = $conexion->conn->nextId("sec_dir_direcciones", $conexion->driver);
//    error_log('................................... '.$nextval);
}
if ($nextval == -1) {
...
//echo "documento $documento_us1 cedu $cc";
    $sgd_ciu_codigo = 'null';
    $sgd_oem_codigo = 'null';
    $sgd_esp_codigo = 'null';
    $sgd_fun_codigo = 'null';
    ...
    //Correccion perdida de remitente en radicados internos
    //Skina
    //Ing Camilo Pintor
    if (!$cc_documento_us1 && $sgd_fun_codigo)
        $cc_documento_us1 = $sgd_fun_codigo;
    
    if ($nextval == '') {
        $consultaID = "select nextval('sec_dir_direccione') as SEC";
        $rsresultado = $conexion->query($consultaID);
        $nextval = $rsresultado->fields['SEC'];
    }
...
//    error_log('# error log linea 207 ' . $nextval);
...
if ($documento_us2) {
//echo "documento $documento_us2 cedu $cc";
    $sgd_ciu_codigo = 'null';
    $sgd_oem_codigo = 'null';
    $sgd_esp_codigo = 'null';
    $sgd_fun_codigo = 'null';
    ...
    if ($rsg->EOF) {
        if ($nextval == -1) {
            ...
            echo "<span class='etextomenu'>No se encontró la secuencia sec_dir_direcciones ";
        }
...
if ($documento_us1 and $cc) {
//echo "documento $documento_us1 cedu $cc linea 299";
    $sgd_ciu_codigo = 'null';
    $sgd_oem_codigo = 'null';
    $sgd_esp_codigo = 'null';
    $sgd_fun_codigo = 'null';
