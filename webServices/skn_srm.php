...
function radicarDocumento($file,$filename,$destinatario,$direccion,$telefono,$tdoc,$no_rm,$no_tran)
{
	...
	*/
	...
	//anexarExpediente($retval,$numExp);	
	//si no existe expediente lo creo
	//crearExpediente($retval,$usuario,$anoExp,$fechaExp,43,13,0,'E');
...
function actualizarDocumento($no_rad,$destinatario,$direccion,$telefono,$email,$tstatus,$no_rm)
{
	...
	*/
	...
	$codusuario=1;
	$observa=$tstatus;
        $txSql = $rs_tx->nrr( $no_rad, $krd,$dependencia,$codusuario,$observa);
...
function consultarDocumento($no_rad,$destinatario,$no_rm,$no_tran,$no_doc)
{
	...
	if($no_rm and $no_rm!='') $Where .= " and r.numero_rm='$no_rm'";
	elseif($no_tran and $no_tran!='') $Where .= " and r.numero_tran='$no_tran'";
	elseif($destinatario and $destinatario!='') $Where .= " and upper(dir.sgd_dir_nomremdes) like '%'"."upper('$destinatario')"."'%'";
	elseif($no_rad and $no_rad!='') $Where .= " and r.radi_nume_radi=$no_rad";
	...
	$sql = "SELECT ".
		...
		FROM sgd_dir_drecciones dir, radicado r, sgd_tpr_tpdcumento td
	...
	$rs = $db->getResult($sql);
	while (!$rs->EOF){
			 $radicado['radicado'] = $rs->fields['RADI_NUME_RADI'];
			 $radicado['fecha']  = $rs->fields['RADI_FECH_RADI'];
			 $radicado['path']  = $rs->fields['PATH'];
			 $radicado['nombre']  = $rs->fields['NOMBRE'];
			 $radicado['direccion']  = $rs->fields['DIRECCION'];
			 $radicado['telefono']  = $rs->fields['TELEFONO'];
			 $radicado['mail']  = $rs->fields['MAIL'];
			 $radicado['no_rm']  = $rs->fields['NO_RM'];
			 $radicado['no_tran']  = $rs->fields['NO_TRAN'];
			 $rs->MoveNext();
	}
	return $radicado;
