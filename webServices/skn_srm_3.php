...
//$ns="http://192.168.20.17/orfeo-3.8.0/webServices/nusoap";
$ns="http://192.168.8.204/orfeo-3.8.0p/webServices/nusoap";
...
***********************************************************************************/
//Adicionando un tipo complejo rad_label 
$server->wsdl->addComplexType(
    ...
    '',
    array(
    'rad' => array('name'=>'rad','type'=>'xsd:string'),
    'label' => array('name'=>'label','type'=>'xsd:string')
    )
);


//Adicionando un tipo complejo consulta 
$server->wsdl->addComplexType(
    'consulta',
    'complexType',
    'struct',
    ...
    'no_tran' => array('name'=>'no_tran','type'=>'xsd:string')
    )
);


//Adicionando un tipo complejo consultas
$server->wsdl->addComplexType(
    'consultas',
    'complexType',
    'array',
...
function crearAnexo($radiNume,$file,$filename,$tipodoc,$nodoc){
	...
	$STDERR = fopen('php://stderr', 'w+');
        fwrite($STDERR, "\n entro a crear anexo radi $radiNume file $file filename $filename tipo $tipodoc no doc $nodoc \n");
	...
	fwrite($STDERR, "numanexos  $numAnexos  \n");
	...
	fwrite($STDERR, "subio anexo $error tipo $tipoAnexo \n");
		...
		fwrite($STDERR, "creo anexo $consulta \n");
	...
	//$cod ? $return='Anexo Creado '.$nombreAnexo : $return='Error';
	$cod ? $return='True' : $return='False';
	fwrite($STDERR, "return $return $error \n");
...
function radicarDocumento($file,$filename,$destinatario,$direccion,$telefono,$tdoc,$no_rm,$no_tran)
{
	...
	//imprimo los datos que recibo
	$STDERR = fopen('php://stderr', 'w+');
 	fwrite($STDERR, " Entro a radicar file $file filename $filename dest $destinatario direccion $direccion tel $tel tdoc $tdoc rm $no_rm tran $no_tran \n");

	//busco el rm que recibo
	...
	if($no_rm!='' and !$rs_rm->EOF){
 		...
 		fwrite($STDERR, "Encontre rm  $grbNombresUs1 \n");
		...
		values ('$no_rm','$grbNombresUs1','$direccion_us1','$telefono_us1',$codep_us1,$muni_us1,1,214,1,8,$id_bod)";	
 		...
 		fwrite($STDERR, "sql bodega $sql_bodega \n");
        }

	
 	fwrite($STDERR, "sql rm $sql_rm $documento_us1 \n");
	...
	$rad->trteCodi =  0;
	if($tdoc and $tdoc!='') $rad->tdocCodi = $tdoc;
	else  $rad->tdocCodi = 0;
	$rad->tdidCodi = 0;
	$rad->carpCodi = 0;
	$rad->carPer = 0;
	...
	$rs = $db->conn->query($sql);
 	...
 	fwrite($STDERR, "sql update rm $sql \n");
	if ($noRad=="-1")
	{
		return new soapval('return','xsd:string',"error $noRad");		
	}
	...
	*/
	...
	if ($nextval==-1)
	{
		return   new soapval('return','xsd:string',"No se encontro la secuencia sec_dir_direcciones ");
	}
	...
	{	case 1:{	//Insercion Exitosa
		...
		case 0:{	//Error Transaccion.
					return  new soapval('return','xsd:string', "No se ha podido actualizar la informacion de SGD_DIR_DRECCIONES UNO ");
				}break;
	}
	unset($record);
	...
	$sql_bexp="select sgd_exp_numero from sgd_sexp_secexpedientes 
			where sgd_exp_numero like '%4313%' and sgd_sexp_parexp1='$no_rm'";
 	$db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
	$rs_bexp=$db->conn->Execute($sql_bexp);		
	$num_expediente=$rs_bexp->fields['SGD_EXP_NUMERO'];
	fwrite($STDERR, "busq exp $sql_bexpediente expediente $num_expediente \n");
	if ($num_expediente and $num_expediente!=''){
				$resultadoExp = $expediente->insertar_expediente($num_expediente,
                                                                ...
                                                                $usua_doc);
	                if($resultadoExp==1) {
				fwrite($STDERR, "se anexo $noRad a $num_expediente\n");
                	}else {
				fwrite($STDERR, "NO se anexo $noRad a $num_expediente\n");
	                }
		}else{
			$fechaExp = date();
			$anoExp = date('Y');
	...
	$label= 'http://192.168.20.17/orfeo-3.8.0/radicacion/gen_etiqueta.php?r='.$noRad;
	$return= array();
	$return['rad']=$noRad;
	$return['label']=$label;
        return new soapval('return','tns:rad_label',$return);
...
function actualizarDocumento($no_rad,$destinatario,$direccion,$telefono,$email,$tstatus,$no_rm)
{
	...
	*/
	...
	fwrite($STDERR, "Entro a actualizar \n nrr $txSql \n");
	//Actualizo datos
	$sql="update sgd_dir_drecciones set sgd_dir_tipo=1";
	if ($destinatario!='') $sql .=" ,SGD_DIR_NOMREMDES='$destinatario'";
	if ($direccion!='') $sql .= " ,SGD_DIR_DIRECCION='$direccion' ";
	if ($telefono!='') $sql .= ",SGD_DIR_TELEFONO='$telefono'";
	if ($email!='') $sql.=" ,SGD_DIR_MAIL='$email'";

	$sql .= "where radi_nume_radi=$no_rad";

	$rs = $db->conn->query($sql);
	fwrite($STDERR, "sqldir $sql\n");

	if ($no_rm!='') {
		$sql_rad="update radicado set numero_rm='$no_rm'
			where radi_nume_radi=$no_rad";
		$rs_rad = $db->conn->query($sql_rad);
	}
	fwrite($STDERR, "sqlrad $sql_rad\n");

	if ($rs) $return='True';
	else $return='False';
	
	fwrite($STDERR, "return $return \n");
...
function consultarDocumento($no_rad,$destinatario,$no_rm,$no_tran,$no_doc)
{
	...
	$STDERR = fopen('php://stderr', 'w+');
        fwrite($STDERR, "radi $no_rad destinatario $destinatario rm $no_rm tran $no_tran doc $no_doc \n");
	...
	elseif($no_doc and $no_doc!='') { $Where.= " and r.radi_nume_radi=a.anex_radi_nume and a.numero_doc='$no_doc'";    $from =", anexos a";  $path="a.anex_nomb_archivo"; }

	if (!$path) $path="r.radi_path";
	...
	while (!$rs->EOF){
		$noRad= $rs->fields['RADI_NUME_RADI'];
		$ano=substr($noRad,0,4);
		$dep=substr($noRad,4,3);
		if($no_doc and $no_doc!='') $path1="bodega/".$ano."/".$dep."/docs/".$rs->fields['PATH'];
		else $path1="/bodega".$rs->fields['PATH'];

			 $radicado[$i] = array ('radicado' => $rs->fields['RADI_NUME_RADI'],
			'info' => "/verradicado.php?verrad=".$rs->fields['RADI_NUME_RADI']."&".session_name()."=".session_id()."&carpeta=8&nomcarpeta=Busquedas&tipo_carp=0",
			 ...
			 'mail'  => $rs->fields['MAIL'],
			 'no_rm'  => $rs->fields['NO_RM'],
			 'no_tran'  => $rs->fields['NO_TRAN'] );
			 $rs->MoveNext();
			 $i++;
	}
	//$obj =new soapval('return','tns:consulta',"$radicado"); 
	//return $obj->serialize();
	 return new soapval('return','tns:consultas',$radicado);
