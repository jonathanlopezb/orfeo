...
class TipoDocumental {
     ...
     * @access public
     */
    function insertarTRD($codiTRDS, $codiTRD, $noRadicado, $coddepe, $codusuario) {  //Arreglo que almacena los nombres de columna
        #==========================
        # Busca el Documento del usuario 
        $ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
        $this->db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT USUA_DOC ,USUA_LOGIN FROM USUARIO WHERE DEPE_CODI='$coddepe' AND USUA_CODI=$codusuario";
        # Busca el usuairo para luego traer sus datos.
        $rs = $this->db->conn->Execute($sql);
        $usDoc = $rs->fields["USUA_DOC"];
        $ADODB_COUNTRECS = true;

        $sql = "SELECT * FROM SGD_RDF_RETDOCF WHERE RADI_NUME_RADI = '$noRadicado' AND  SGD_MRD_CODIGO =  '$codiTRD'";
    ...
    function insertarAnexoTRD($noRadicado, $serie, $subserie) {  //Arreglo que almacena los nombres de columna
        $anexosRadicado = "select ANEX_CODIGO from anexos where anex_radi_nume='$noRadicado'";
    ...
    function insertarTRDA($codiTRDS, $codiTRD, $noRadicado, $noRadicadoA, $coddepe, $codusuario) {  //Arreglo que almacena los nombres de columna
        #==========================
        # Busca el Documento del usuario 
        $ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
        $this->db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
        ...
        $rs = $this->db->conn->Execute($sql); # Executa la busqueda y obtiene el registro a actualizar.
        if ($rs->RowCount() >= 1)
            die("<HR><center><B><FONT COLOR=RED>Esta Tipificacion YA esta incluida. <BR>  VERIFIQUE LA INFORMACION E INTENTE DE NUEVO</FONT></B></center><HR>");
        $record = array(); # Inicializa el arreglo que contiene los datos a insertar
        # Asignar el valor de los campos en el registro
        # Observa que el nombre de los campos pueden ser mayusculas o minusculas
        foreach ($codiTRDS as $codiTRDR) {
    ...
    function eliminarTRD($nurad, $coddepe, $usua_doc, $codusuario, $codiTRD) {
        ...
        $isqlE = "delete from SGD_RDF_RETDOCF where RADI_NUME_RADI='$nurad' and SGD_MRD_CODIGO = $codiTRD";
        $rsE = $this->db->conn->Execute($isqlE);

        /* Tipo Documento del Radicado */
        $sql = "SELECT  TDOC_CODI FROM radicado WHERE radi_nume_radi = '$nurad'";
        $rs = $this->db->conn->Execute($sql);
        $tip_dcto = $rs->fields['TDOC_CODI'];

        /* Tipo Documento de la Calsificacion Eliminada */
        $sql = "select SGD_TPR_CODIGO from SGD_MRD_MATRIRD where SGD_MRD_CODIGO = $codiTRD";
        $rs = $this->db->conn->Execute($sql);
        ...
        if ($tip_trd == $tip_dcto) {
            ...
            $isqlM = "select $radi_nume_radi RADI_NUME_RADI, SGD_MRD_CODIGO from SGD_RDF_RETDOCF r where r.RADI_NUME_RADI='$nurad'";
            ...
            if ($codiTRDM != '') {
                ...
                $isqlM = "select SGD_TPR_CODIGO from SGD_MRD_MATRIRD where SGD_MRD_CODIGO = '$cod_nvo'";
                $rsM = $this->db->conn->Execute($isqlM);
    ...
    function eliminarTRDA($nurad, $coddocu, $coddepe, $usua_doc, $codusuario, $codiTRD) {
        ...
        $isqlE = "select rownum as NUM, ANEX_RADI_NUME, ANEX_CODIGO, SGD_MRD_CODIGO from SGD_RDA_RETDOCA where ANEX_RADI_NUME='$nurad' and ANEX_CODIGO='$coddocu'";
        $rsE = $this->db->conn->Execute($isqlE);
        ...
        if ($rsE->RowCount() > 1) {
            ...
            if ($numreg_Eli == $rsE->RowCount()) {
                $i = $rsE->RowCount() - 1;
                $isqlE = "select ANEX_RADI_NUME, ANEX_CODIGO, SGD_MRD_CODIGO from SGD_RDA_RETDOCA where ANEX_RADI_NUME='$nurad' and ANEX_CODIGO='$coddocu' and rownum = '$i'";
                $rsE = $this->db->conn->Execute($isqlE);
                $cod_nvo = $rsE->fields['SGD_MRD_CODIGO'];

                $isqlE = "select SGD_TPR_CODIGO from SGD_MRD_MATRIRD  where SGD_MRD_CODIGO = '$cod_nvo'";
                $rsE = $this->db->conn->Execute($isqlE);
        ...
        if ($indi_change == "SI") {
            $sql = "SELECT SGD_TPR_CODIGO FROM anexos WHERE  ANEX_RADI_NUME = '$nurad' and ANEX_CODIGO = '$coddocu'";
            $rs = $this->db->conn->Execute($sql);
        ...
        $isqlEAnex = "delete from SGD_RDA_RETDOCA where ANEX_RADI_NUME='$nurad' and ANEX_CODIGO='$coddocu' and SGD_MRD_CODIGO = '$codiTRD'";
    ...
    function calcFechVenc($fldTermino, $fldFechInic) {
		...
		";
            $rs2 = $db2->conn->Execute($isql_busca);
    ...
    function suma_fechas($fecha, $ndias) {
        list($ano, $mes, $dia) = preg_split("/[\s-]+/", $fecha);
        if ($mes == 'JAN')
            $mesn = 1;
        else
        if ($mes == 'FEB')
    ...
    function actualizarTRD($radicados, $tdoc) {
        ...
        foreach ($radicados as $noRadicado) {
            $sql = "SELECT TDOC_CODI FROM radicado WHERE radi_nume_radi =' " . $noRadicado . "'";
