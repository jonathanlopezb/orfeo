...
/**
 * Contactos es la clase encargada de gestionar la informaci�n referente a los contactos de empresas
...
class Contactos {
    ...
    /**
     * Funci�n encargado de crear un recordset con los contactos de una empresa.
     * @param	$tipo	0=Entidad  1=Empresas
     * @param	$id_emp	Id de la empresa a buscar contactos
     * @return   recordset
     */
    function &SelectContactos($tipo, $id_emp) { //$tipo += 1;
        ...
        $Rs_ctt = $this->cursor->conn->CacheExecute(15, $sql);
        unset($sql);
        return $Rs_ctt;
    }

    /**
     * Funci�n encargado de crear un vector asocciativo con �ndice incremental.
     ...
     */
    function &GetMenuCnt($tipo_tb, $id_emp, $id_defa) { // 1er par�metro. ===> 0= Entidad(Bodega_empresas)    1 = Empresas (SGD_OEM_OEMPRESAS)
