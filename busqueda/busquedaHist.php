...
//Modificado skina - Rango de un mes
	...
	$adodb_next_page or $orderNo or $orderTipo or $orden_cambio)
{
	...
	if($palabra)
	{
	 ...
	 if($palabra)
        {
        $ps_RADI_NOMB = strtoupper($palabra);
        $concatenacion="UPPER(".$db->conn->Concat("R.RA_ASUN","R.RADI_CUENTAI","A.SGD_DIR_TELEFONO","A.SGD_DIR_DIRECCION") . ") LIKE '%";
        $tok= explode(" ",$ps_RADI_NOMB);
                $sWhere.=" AND ((UPPER(A.sgd_dir_nomremdes) LIKE '%".implode("%' AND UPPER(A.sgd_dir_nomremdes) LIKE '%",$tok)."%') ";
                $sWhere .= " OR (".$concatenacion.implode("%' AND ".$concatenacion,$tok)."%'))";
        }
	...
	$sql = 'SELECT R.RADI_NUME_RADI AS "IMG_Numero Radicado",
			...
			GROUP BY R.RADI_NUME_RADI, R.RADI_PATH, R.RADI_NUME_RADI, ".$sqlFecha.", A.SGD_DIR_NOMREMDES, A.SGD_DIR_DOC, R.RA_ASUN";
	...
	$sql = 'SELECT R.RADI_NUME_RADI AS "IMG_Numero Radicado",
				...
				TTR.SGD_TTR_DESCRIP AS "Transaccion",'.
			...
			FROM RADICADO R INNER JOIN
				...
				DEPENDENCIA D ON R.RADI_DEPE_ACTU = D.DEPE_CODI INNER JOIN SGD_TTR_TRANSACCION TTR ON H.SGD_TTR_CODIGO=TTR.SGD_TTR_CODIGO
			WHERE '.
				$db->conn->SQLDate('Y/m/d','H.HIST_FECH'). ' BETWEEN \''.$fecha_ini.'\' and \''.$fecha_fin.'\'
				AND A.SGD_DIR_TIPO=1 AND H.USUA_CODI = '.$_SESSION["codusuario"].
				' AND H.DEPE_CODI = '.$_SESSION["dependencia"].' '.$sWhere.'
			GROUP BY R.RADI_NUME_RADI, R.RADI_PATH, R.RADI_NUME_RADI, '.$sqlFecha.', A.SGD_DIR_NOMREMDES,R.RA_ASUN,ttr.sgd_ttr_descrip, 
...
<tr>
	<td valign="top">
		...
		<tr>
			...
			<!-- Modificado skina 070909	<INPUT type="radio" NAME="s_solo_nomb" CHECKED value="All" <? if($s_solo_nomb == "All") { echo ("CHECKED");} ?>>Todas las palabras (y)<br>
