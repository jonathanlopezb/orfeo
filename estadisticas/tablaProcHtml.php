...
<CENTER>
   <table border="0" cellpadding="0" cellspacing="2" class="borde_tab">
	...
	if($fld->name=="RADICADO") 
	{
		...
		if(trim($pathImg)) 
		{
			echo "<a href=$dir_raiz/bodega/$pathImg>";
		}
	}
	...
	if(substr($fld->name,0,3)=="DAT") 
	{
      ...
      $ImpVerRadicado="<a href=$dir_raiz/verradicado.php?verrad=$numRadicado&krd=$krd&".$encabezado." target=".$numRadicado.">Ver Inf. </a>";
      echo $ImpVerRadicado;
	}		
	...
	if(!$genDetalle)
	{
		if($genTodosDetalle==1)  {
			...
			<img src="<?=$dir_raiz?>/<?=$imagenes?>/investigaciones.jpeg" width=10 height=10  alt="<?=$descTitulo?>" title="<?=$descTitulo?>">
...
//$nombUs = array("ddd","kuiyiuiu","kjop99");
//$data1y = array(11,23,45);
$nombreGraficaTmp = "$dir_raiz/bodega/tmp/E_$krd.png";
