    ...
    <head>
        <script language="JavaScript">
            ...
            function MM_validateForm() { //v4.0
                ...
                if (errors)
                    alert('Asegurese de entrar el password Correcto, \n No puede ser Vacio:\n');
...
$usualogin = isset($rs->fields["USUA_LOGIN"]) ? $rs->fields["USUA_LOGIN"] : $rs->fields["usua_login"];
        
if (trim($usualogin) == $krd) {
    ...
    ?>
        <body>
        <center>
            <form action='usuarionuevo.php?<?= session_name() . "=" . session_id() ?>&krd=<?= $krd ?>' method=post onSubmit="MM_validateForm('contradrd', '', 'R', 'contraver', '', 'R');
                        return document.MM_returnValue">
                <div class="flotante">
                    <h2>Cambio de contrase&ntilde;a Usuarios</h2>
                    <h4>Por favor introduzca la nueva contrase&ntilde;a</h4>
                    <div class="row">
                        <div class="col-xs-12 col-sm-3 col-md-3 col-lg-3 box">
                            <input type=hidden name='usuarionew' value="<?= $krd ?>">
                            <p>Usuario</p>
                            <input type="text" name="" value="<?= $krd ?>" readonly="">
                        </div>
                        <div class="col-xs-12 col-sm-3 col-md-3 col-lg-3 box">
                            <p>Nueva contrase&ntilde;a</p>
                            <input type=password name=contradrd vale='' class=tex_area title='Ingrese nueva contrase&ntilde;a para el usuario <?= $krd ?>'>
                        </div>
                        <div class="col-xs-12 col-sm-3 col-md-3 col-lg-3 box">
                            <p>Confirmar la contrase&ntilde;a</p>
                            <input type=password name=contraver class=tex_area vale='' title='Ingrese nuevamente su contrase&ntilde;a para el usuario <?= $krd ?>'>
                        </div>
                    </div>
                    <input type=hidden value="<?= $depsel ?>" name=depsel>
                    <button type="submit" name="button" class="but">Cambiar contrase&ntilde;a</button>
                    <br>
                </div>
        </center>
