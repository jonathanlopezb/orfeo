...
function PRESTAMO_action($sAction) {
    ...
    if ($sAction == "cancelar") {
        ...
        $fldPRES_DEPE_ARCH = substr($fldradicado, 4, $longitud_codigo_dependencia);
        ...
        // Genera PRES_ID
        //$db->conn->debug = true;
        $sec = $db->conn->nextId('SEC_PRESTAMO', $db->driver);
        //$db->conn->debug = true;
        print $fldPRES_DEPE_ARCH;
        $sSQL = "insert into PRESTAMO(
               ...
               values (" .
                ...
                tosql($krd, "Text") . "," .
                tosql(str_pad($dependencia, $longitud_codigo_dependencia, '0', STR_PAD_LEFT), "Text") . "," .
                $fldPRES_FECH_PEDI . "," .
                tosql($fldPRES_DEPE_ARCH, "Text") . ", 
                  1," .
...
function ESTADO_PRESTAMO_show() {
    ...
    if ($sPRESTAMOErr == "") {
        ...
        $sSQL = "select 
		distinct(r.radi_nume_radi) as RADICADO,
        ...
        if ($rs && !$rs->EOF) {
            ...
            <form method="POST" action="<?= $sFileName ?>" name="Prestados">
                ...
                <table border='1' cellpadding=0 cellspacing=2 class='borde_tab'>
                    <tr>
...
//$sqlReq = "aaaa";
        if (strlen($sqlReq) != 0) {
            ...
            <form method="POST" action="<?= $sFileName ?>" name="Prestamo">
                ...
                ?>  
                <center>
                    <div id="titulo" style="width: 100%;" align="center"><?= $sFormTitle ?></div>
                </center>
                <table class="borde_tab" align="center" border='1'>
                <!--                     <tr>
                      <td class="titulos2" colspan="2"><center><?= $sFormTitle ?></center></td>
                   </tr>-->
                    <tr>
                        <td class='titulos2' width="100">Radicado</td>
                        <td class='listado2' width="250"><?= $fldradicado ?></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td class='listado2' colspan="2" style="text-align: center">
                            <?php 
                            if (isset($_GET['sExpediente']) && $_GET['sExpediente'] == 'si') {
                                ?>
                                <input type="submit" class="botones" value="Solicitar">&nbsp;&nbsp;
                                <input type="submit" class="botones" value="Regresar" onClick="javascript: regresar();">
                                <?php
                            }else{
                                echo '<span style="color:red">No se puede realizar la solicitud del préstamo porque el radicado no se encuentra '
                                . 'en un expediente. Primero debe hacer el proceso de inclusión de expediente.</span>';
                            }
                            ?>
                        </td>	
                    </tr>
