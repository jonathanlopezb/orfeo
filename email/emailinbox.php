...
$response = $msg->getHeadersEmailsInSequence($inbox_email, $minValue, $maxValue, true);
//print_r($response);
                    ...
                    <table border="1" class="borde_tab" width="99%">    
                        ...
                        <?php foreach($response['headers'] as $header): ?>
                            ...
                            <tr class="listado1">
                                ...
                                <td class="msgitem">
                                    <?php if(isset($attachments[$msgno]) && count($attachments[$msgno])): ?>
                                        <ul>
                                            <?php foreach($attachments[$msgno] as $attachment): ?>
                                                <?php if(isset($attachment['is_attachment']) && $attachment['is_attachment'] == true): ?>
                                                    <li><a href='attachement.php?msgno=<?= $msgno ?>&numadj=<?= $attachment['id'] ?>&folder=<?= $inbox_email ?>' target='formulario'><img src='imagenes/attach.png' width='18' height='18' border='0' alt='<?= $attachment['filename'] ?> ' title=' <?= $attachment['filename'] ?> '>&nbsp;<?= imap_utf8($attachment['filename']) ?></a></li>          
                                                <?php endif; ?>    
                                            <?php endforeach ?>
                                        </ul>
                                    <?php endif; ?>   
