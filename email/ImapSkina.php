...
class ImapSkina{
   ...
   /**
    ...
    public function getHeadersEmailsInSequence($folder = 'INBOX', $minVal, $maxVal, $getAttachments = true){
        ...
        if($getAttachments){
            ...
            foreach($headers as $header){
                $attachments[$header->msgno] = $this->getAttachments($imap, $header->msgno, $options);
            }
        }
    ...
    public function getEmailInfo($messageId, $folder = 'INBOX')
    {
        ...
        $emailStructure = imap_fetchstructure($imap, $messageId, $options);

        $emailAttachments = $this->getAttachments($imap, $messageId, $options);
