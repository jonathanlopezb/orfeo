...
function Header()
{
	...
	$this->SetXY($l,20+$margen);
 	$this->Cell(407,14,strtoupper($this->entidad_largo),1,1,'L');
	$this->SetXY($l,34+$margen);
 	$this->Cell(407,13,'',1,1,'L');
	$this->SetXY($l,47+$margen);
 	$this->Cell(407,13,'CONTRATO No ',1,1,'L');
	$l = 435;
	$this->SetXY($l,20+$margen);
	...
	if (!$this->rs->EOF)  {
		...
		if($i_datos>=$row_ini and $i_datos<=($row_fin))
			{
				...
				$gp    = $this->rs->fields[14];    //	   $gp = trim(ora_getcolumn($this->cursor,14));
				$gp1   = $this->rs->fields[2]; //  $gp1 =trim(ora_getcolumn($this->cursor,2));
				$gptot = $this->rs->fields[13];	 //  $gptot = trim(ora_getcolumn($this->cursor,13));
		...
		{ 
			...
			{	
				...
				if($col==13) $txt = $total;	
				if($col==0) 	$txt = $this->rs->FetchInto($txt);		//$txt=$row; 
					$this->page = $currpage;
					$this->SetXY($l,$h); 
