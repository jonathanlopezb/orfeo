...
//		$this->db->conn->debug = true;
    ...
    function consulta_exp($radicado) {
        switch ($this->db->driver) {
            case 'mssql':$radi_nume_radi = " e.radi_nume_radi";
                break;
            default:$radi_nume_radi = "e.radi_nume_radi";
                break;
        }
    ...
    function insertar_expediente($num_expediente, $radicado, $depe_codi, $usua_codi, $usua_doc) {
        ...
        if (!$rs->fields["SGD_EXP_NUMERO"]) {
            ...
            $query = "select e.SGD_EXP_NUMERO, e.SGD_EXP_FECH from SGD_EXP_EXPEDIENTE e WHERE e.SGD_EXP_NUMERO='$num_expediente' order by e.SGD_EXP_FECH DESC";
        ...
        $q_excluido = "SELECT SGD_EXP_ESTADO FROM SGD_EXP_EXPEDIENTE WHERE SGD_EXP_NUMERO = '" . $num_expediente . "' AND RADI_NUME_RADI = '" . $radicado . "'";
        $rs_excluido = $this->db->conn->query($q_excluido);

        error_log('@@@@@ '.$q_excluido);  

        if($rs_excluido->fields["SGD_EXP_ESTADO"]){
            if ($rs_excluido->fields["SGD_EXP_ESTADO"] == 2) {

                $q_update = "UPDATE SGD_EXP_EXPEDIENTE SET SGD_EXP_ESTADO = 0 WHERE SGD_EXP_NUMERO = '" .$num_expediente. "' AND RADI_NUME_RADI = '" . $radicado . "' AND SGD_EXP_ESTADO = 2";
                error_log('@@@@@ '.$q_update);    
                if (!$rs_update = $this->db->conn->query($q_update)) { return 0; } else {return 1;}
            } 
            else {return 0;}
        }
    ...
    function crearExpediente($numExpediente, $radicado, $depe_codi, $usua_codi, $usua_doc, $usuaDocExp, $codiSRD, $codiSBRD, $expOld = null, $fechaExp = null, $codiPROC = null, $arrParametro = null) {
        ...
        if (is_array($arrParametro)) {
            foreach ($arrParametro as $orden => $datoParametro) {
                ...
                $valorParametro .= "'" . $datoParametro . " ($depe_codi)'" . $coma;
                $p++;
            }
        }

        $estado_expediente = 0;
        $query = "select SGD_EXP_NUMERO, DEPE_CODI from SGD_SEXP_SECEXPEDIENTES WHERE SGD_EXP_NUMERO='$numExpediente'";
        ...
        if ($expOld == "false") {            
            ...
            if ($expManual == 1) {
                ...
                $secExp = substr($numExpediente, 11, 7);
            }

            $consecutivoExp = str_pad($secExp, 6, "0", STR_PAD_LEFT);
            $numeroExpediente = $anoExp . str_pad($dependencia, $longitud_codigo_dependencia, '0', STR_PAD_LEFT) . $trdExp . $consecutivoExp;
        ...
        if ($rs->fields["SGD_EXP_NUMERO"] == $numExpediente && $rs->fields["DEPE_CODI"] == $dependencia) {
            return 0;
        } else {

            $queryNombre = "select SGD_EXP_NUMERO, DEPE_CODI from SGD_SEXP_SECEXPEDIENTES WHERE sgd_sexp_parexp1	='".trim($valorParametro)."'";
            $rsNombre = $this->db->conn->Execute($queryNombre);

            if ($rsNombre->fields["SGD_EXP_NUMERO"] == $numExpediente && $rsNombre->fields["DEPE_CODI"] == $dependencia) {
                return 0;
            }
            else{
                ...
                $sex_exp_max = "SELECT SGD_SEXP_SECUENCIA FROM SGD_SEXP_SECEXPEDIENTES ORDER BY SGD_SEXP_SECUENCIA DESC LIMIT 1";
                $rs = $this->db->conn->Execute($sex_exp_max);

                $secExp = $rs->fields["SGD_SEXP_SECUENCIA"] + 1;
...
//$this->db->conn->debug=true;
        ...
        if ($rs) {
            ...
            $this->exp_subexpediente = $this->rs->fields['SGD_EXP_SUBEXPEDIENTE'];
...
//			die("Cuenta 1: " . $rs->FieldCount()  );
                ...
                $query = "SELECT "
                        ...
                        . ", SGD_FEXP_TERMINOS "
                        . "FROM SGD_FEXP_FLUJOEXPEDIENTES "
                        . "WHERE SGD_PEXP_CODIGO= " . $this->codigoTipoExp. " ORDER BY SGD_FEXP_ORDEN ";
...
//            	die("Cuenta 2: " . $rs->FieldCount()  );
    ...
    function secExpediente($dependencia, $codiSRD, $codiSBRD, $anoExp) {
        $query = "SELECT COUNT(sgd_exp_numero) as TOTALEXP FROM sgd_sexp_secexpedientes";
        $rs = $this->db->conn->Execute($query);
        $secExp = $rs->fields["TOTALEXP"];
        if (!$secExp) {
            $secExp = 1;
        } else {
            $secExp = $secExp + 1;
    ...
    function expedientesRadicado($radicado) {
        $query = "SELECT SGD_EXP_NUMERO FROM SGD_EXP_EXPEDIENTE WHERE RADI_NUME_RADI = '" . $radicado . "' AND SGD_EXP_ESTADO <> 2";
        $rs = $this->db->conn->query($query);
        if ($rs->EOF) {
            $arrExpedientes[0] = 0;
        } else {
    ...
    function grabarSubexpediente($radicado, $numExpediente, $Subexpediente) {
        //by skina para ciac 13-07-11
        //agregamos comillas (varchar)
        $query = "UPDATE SGD_EXP_EXPEDIENTE";
    ...
    /**
     * FUNCION PARA GRABAR EL HISTORICO DEL EXPEDIENTE
     * Guarda la trazabilidad de los radicados 
     * @param $num_exp Numero expediente
     * @param $num_rad Numero radicado
     * @param $usua_doc Numero de documento que registra
     * @param $usua_codi codigo de usuario que registra
     * @param $depe_codi codigo de dependencia del usuario que registra
     * @param $cod_tx codigo de transaccion
     * @param $oberva observaciones
     * */
    function insertHistoricoExpe($num_exp, $num_rad, $usua_doc, $usua_codi, $depe_codi, $cod_tx, $oberva) {

        // $sqlFechaHoy = $db->conn->DBDate(date('Y-m-d'));
        $sqlFechaHoy = $this->db->conn->OffsetDate(0, $this->db->conn->sysTimeStamp);

        $query = 'INSERT INTO SGD_HFLD_HISTFLUJODOC ( SGD_FEXP_CODIGO, SGD_HFLD_FECH, SGD_EXP_NUMERO, RADI_NUME_RADI, USUA_DOC, USUA_CODI, DEPE_CODI, SGD_TTR_CODIGO, SGD_HFLD_OBSERVA ) ';
        $query .= "VALUES ( 0, " . $sqlFechaHoy . ", '" . $num_exp . "', '" . $num_rad . "', '" . $usua_doc . "', '" . $usua_codi . "', '" . $depe_codi . "', '" . $cod_tx . "', '" . $oberva . "' )";
        $this->db->conn->query($query);
    }
