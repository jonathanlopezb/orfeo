...
// end of member function consultaHistoricos
     ...
     *
     */
    function insertarHistorico($radicados, $depeOrigen, $usCodOrigen, $depeDestino, $usCodDestino, $observacion, $tipoTx, $result = null) {
        
        //error_log(' ### '.$radicados.','. $depeOrigen.','.$usCodOrigen.','.$depeDestino.','.$usCodDestino.','.$observacion.','.$tipoTx);
        ...
        $sql = "SELECT USUA_DOC ,USUA_LOGIN FROM USUARIO WHERE DEPE_CODI='$depeOrigen' AND USUA_CODI=$usCodOrigen";
        ...
        $sql = "SELECT USUA_DOC ,USUA_LOGIN FROM USUARIO WHERE DEPE_CODI='$depeDestino' AND USUA_CODI=$usCodDestino";
        $rs = $this->db->conn->Execute($sql);
        $usDocDestino = $rs->fields["USUA_DOC"];
        $usuActu = $rs->fields["USUA_LOGIN"];
        //echo 'valor de isql en linea 78 '.$sql;
        $record = array(); # Inicializa el arreglo que contiene los datos a insertar
        if ($radicados) {
            if ($result != null) {
                $record["USUA_DOC_OLD"] = $result;
            }
            foreach ($radicados as $noRadicado) { # Asignar el valor de los campos en el registro
                # Observa que el nombre de los campos pueden ser mayusculas o minusculas
                if (!$usDocDestino)
                    $usDocDestino = "0";
                $record["RADI_NUME_RADI"] = (string) $noRadicado;
...
// end of member function insertarHistorico
    ...
    function insertarHistoricoArch($id, $radicados, $depeOrigen, $usCodOrigen, $observacion, $tipoTx) {
        ...
        $ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
        $this->db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT  USUA_DOC FROM USUARIO WHERE DEPE_CODI='$depeOrigen' AND USUA_CODI=$usCodOrigen";
        # Busca el usuairo Origen para luego traer sus datos.
        //$this->db->conn->debug=true;
        $rs = $this->db->conn->Execute($sql);
        $usDocOrigen = $rs->fields["USUA_DOC"];
...
// end of member function insertarHistorico
    ...
    function insertarHistoricoExp($numeroExpediente, $radicados, $dependencia, $codUsuario, $observacion, $tipoTx, $codigoFldExp) {
        ...
        $ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
        $this->db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
        $sql = "SELECT USUA_DOC, USUA_LOGIN FROM USUARIO WHERE DEPE_CODI='$dependencia' AND USUA_CODI=$codUsuario";
        # Busca el usuairo Origen para luego traer sus datos.
        $rs = $this->db->conn->Execute($sql);
