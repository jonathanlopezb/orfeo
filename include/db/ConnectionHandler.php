...
//Almacena un error, resultado de una transacci�
    ...
    function ConnectionHandler($dir_raiz) {
        if (!defined('ADODB_ASSOC_CASE'))
            define('ADODB_ASSOC_CASE', 1);
        include ("$dir_raiz/adodb/adodb.inc.php");
        include_once ("$dir_raiz/adodb/adodb-paginacion.inc.php");
        include_once ("$dir_raiz/adodb/tohtml.inc.php");
        include ("$dir_raiz/config.php");
        $ADODB_COUNTRECS = false;
        $this->driver = $driver;
        $this->conn = NewADOConnection("$driver");
        $this->dirRaiz = $dir_raiz;
        $this->rutaRaiz = $_SESSION['dir_raiz'];
        //$this->rutaRaiz = $dirraiz;
