...
<html>
    ...
    Fecha: 08-11-2018
    Información: Eliminado (onLoad="window_onload();") del body, ya que esa función no existe
    -->
...
//        $db->conn->debug = true;
        ...
        <form name=formEnviar action='crear.php?<?= $encabezado ?>' method=GET>
            ...
            include "$dir_raiz/include/query/administracion/queryCuerpoEdicion.php";

//            $db->conn->debug = true;
//            $rs = $db->conn->Execute($isql);
            $nregis = $rs->fields["usua_nomb"];
            if ($rs->EOF) {
                echo "<hr><center><b><font color='red'>No hay resultados con el criterio de b&uacute;squeda</font></center></b></hr>";
            } else {
