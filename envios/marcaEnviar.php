...
//        $db->conn->debug=true;
        ...
        ?>
        <br>
<!--        <table class="borde_tab" width="98%" cellspacing="5" align="center">
        ...
        <form name='forma' id="forma" action='marcaEnviar.php?<?= session_name() . "=" . session_id() . "&fecha_h=$fechah&dep_sel=$dep_sel&estado_sal=$estado_sal&estado_sal_max=$estado_sal_max&no_planilla=$no_planilla" ?>' method=post>
            <center>
                <table border=2 width=98% align="center" class=borde_tab cellspacing="5">
                    <tr>
                    <div id="titulo" style="width: 98%;" align="center">Impresi&oacute;n de Documentos</div>
                    </tr>
                    <tr class="titulos2">
                        ...
                        while ($i < $num) {
                            ...
                            if ($estadoFirma == "COMPLETA") {
                                ...
                                if ($swValido == false) {
                                    ...
                                    copy("$dir_raiz/$linkarchivo", "$dir_raiz/bodega/masiva/$nombreArchivo");
                                    $fp = fopen("$ir_raiz/bodega/masiva/$archInsumo", 'w+');
...
//                $ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
                    include "$dir_raiz/include/query/envios/queryMarcaEnviar.php";
                    $db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
                    $rsMarcar = $db->query($isql);
//echo 'valor de isql '.$isql;
//$no_registros = $rsMarcar->recordcount(); 
//                error_log('----------- '. print_r($rsMarcar->fields).' ----');
                    $verRegistros = "";
                    ...
                    if (!$verRegistros) {
                        ...
                        echo '<script language="javascript">alert("No se puede Marcar el Documento ' . $verrad_sal . ' Como Impreso. ' . $mensaje . '  ");</script>';
                    } else {
                        while (!$rsMarcar->EOF) {
                            $mensaje = "";
...
//                        $asunto = substr($rsMarcar->fields["ANEX_DESC"],1,300);
