...
//                            echo 'paso aqui linea 608 <br> ';
            ...
            <td >
            <?php
             if( $rs->fields["RADI_NUME_SALIDA"] && $rsF->fields["FIRMA_QR"] == 1 && $rs->fields["DOC_FIRMADO_QR"] == 0 ){ 
            ?>
                <font size=1>
                    <center>
                    <?php echo "<a class=vinculos href=javascript:firmaRadicadoQR('$coddocu','$linkarchivo','$cod_radi');  aria-label='firmar el documento radicado al'><span style='color: #CE5622; font-size: 18px;' class='glyphicon glyphicon-edit' id='tooltips' data-toggle='tooltip' title='Firmar radicado.' aria-hidden='true'> </span></a>"; ?>
                    </center>
                </font> 

            <?php } 
                if ( $rs->fields["DOC_FIRMADO_QR"] == 1 ){
            ?>
                <font size=2>
                    <center>
                    <span style='color: #CE5622; font-size: 18px;' class='glyphicon glyphicon-check' id='tooltips' data-toggle='tooltip' title='Firmado.' aria-hidden='true'> </span></center>
                </font>
            <?php
                }
            ?>
