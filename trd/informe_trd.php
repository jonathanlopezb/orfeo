    ...
    <form name="inf_trd"  action='../trd/informe_trd.php?<?= session_name() . "=" . session_id() . "&krd=$krd&fecha_h=$fechah" ?>' method=post>
        <center>
            ...
            <table border='1' class='borde_tabReducido' style="font-size: 10px">
                ...
                <tr>
                    <td rowspan="3" class=titulos3>C&oacute;digo</td>
                    <td>D: C&oacute;digo de la dependencia</td>
                    <td rowspan="2" class=titulos3>Retenci&oacute;n (A&ntilde;os)</td>
                    <td>AG: Archivo gesti&oacute;n</td>
                </tr>
                <tr>
                    <td>S: Serie</td>                    
                    <td>AC: Archivo central</td>
                </tr>
                <tr>
                    <td>Sb: Subserie</td>
                    <td rowspan="5" class=titulos3>Soporte</td>
                    <td>P: Papel</td>                    
                </tr>
                <tr>                    
                    <td rowspan="4" class=titulos3>Disposici&oacute;n final</td>
                    <td>CT: Conservaci&oacute;n total</td>
                    <td>EL: Electronico</td>
                </tr>
                <tr>
                    <td>E: Eliminaci&oacute;n</td>
                    <td>O: Otro</td>

                </tr>
                <tr>
                    <td>MT: Medio Tecnol&oacute;gico</td>
                </tr>
                <tr>
                    <td>S: Selecci&oacute;n</td>
                </tr>
            </table>
        </center>
...
//parametro solo de la SSPD
            ...
            $order_isql = " order by m.depe_codi, m.sgd_srd_codigo,m.sgd_sbrd_codigo";
            ...
            <table class='borde_tabReducido' border='2' width='98%' align='center' >
                ...
                <tr class=titulos3>
                    ...
                    <td colspan="2" align="center">Retenci&oacute;n<br> A&#241;os</td>
                ...
                <tr class=titulos3>
                    ...
                    <td>CT</TD><TD>E</TD><TD>MT</TD><TD>S</TD>
                    <td>P</td><td>EL</td><td>O</td>
                </tr>
                <?php
                while (!$rs->EOF and $rs) {

                    $nSRD = $assoc == 0 ? strtoupper($rs->fields['sgd_srd_descrip']) : strtoupper($rs->fields['SGD_SRD_DESCRIP']); //Nombre Serie
                    $depTDR = $assoc == 0 ? $rs->fields['depe_codi'] : $rs->fields['DEPE_CODI'];     //Dependencia
                    $depTDR_nomb = $assoc == 0 ? $rs->fields['depe_nomb'] : $rs->fields['DEPE_NOMB'];     //Dependencia
                    $nSBRD = $assoc == 0 ? $rs->fields['sgd_sbrd_descrip'] : $rs->fields['SGD_SBRD_DESCRIP'];   //Nombre SubSerie
                    $cSRD = $assoc == 0 ? $rs->fields['sgd_srd_codigo'] : $rs->fields['SGD_SRD_CODIGO'];    //Codigo Serie
                    $cSBRD = $assoc == 0 ? $rs->fields['sgd_sbrd_codigo'] : $rs->fields['SGD_SBRD_CODIGO'];   //Codigo Subserie
                    //by skina php5.3
                    $nTDoc = $assoc == 0 ? ucfirst($rs->fields['sgd_tpr_descrip']) : ucfirst($rs->fields['SGD_TPR_DESCRIP']); //Nombre Tipo Documental
