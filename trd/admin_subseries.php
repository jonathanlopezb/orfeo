...
if (isset($_POST['actua_subserie'])) {
    ?>
                            <!--<td>--><input type=submit name=modi_subserie Value='Grabar Modificación' class=botones_largo style='margin-left: 11%; margin-top: 5px;  float: left; position: absolute;'><!--</td>-->
    <?
}
?>
                    <tr>
                        <td  class='titulos2'><label for="tsub">C&oacute;digo Subserie</label></td>
                        ...
                        <td    class='titulos2'><label for="detasub">Descripci&oacute;n</label></td>
                        <td valign="top" align="left" class='listado2'>
            ...
            dateAvailable.writeControl();
                    ...
                    <tr> 
                        <TD class='titulos2'><label for="tiem_ag"> Tiempo Archivo de Gesti&oacute;n</label></td>
                        <TD valign="top" align="left" class='listado2'>
                    ...
                    <tr> 
                        ...
                        <TD class='titulos2'><label for="med">Disposici&oacute;n Final</label></td>
                        <TD valign="top" align="left" class='listado2'>
...
//$detasub = strtoupper(trim($detasub));
					  ...
					  ";
                                ...
                                if ($radiNumero != '') {
                                    echo "<script>alert('El código ya existe, por favor verifique la información e intente de nuevo');</script>";
                                } else {
                                    $isqlB = "select * from sgd_sbrd_subserierd 
					  			where sgd_srd_codigo = '$codserie'
						  ...
						  ";
                                    ...
                                    $radiNumero = $rs->fields["SGD_SRD_CODIGO"];
                                    if ($radiNumero != '') {
                                        echo "<script>alert('La serie ya existe. Verifique la información e intente de nuevo');</script>";
                                    } else {
						...
						VALUES ($codserie,$tsub,'$detasub'    ," . $sqlFechaD . "," . $sqlFechaH . ",$tiem_ag,$tiem_ac,'$med','$soporte','$asu')";
                                        ...
                                        $soporte = '';
                                        if (!rsIN){
                                            echo "<script>alert('Verifique todos los datos.');</script>";
                    					}else{
                                            echo "<script>alert('Se creo correctamente la subserie');</script>";
                    					}
                                        ?>
                                        <script language="javascript">
                        ...
                        if (isset($_POST['actua_subserie'])) {
                            if ($codserie != 0 && $tsub != 0) {
                                $isqlB = "select * from sgd_sbrd_subserierd where sgd_srd_codigo = $codserie and sgd_sbrd_codigo = $tsub ";
                                # Selecciona el registro a actualizar
                                $rs = $db->query($isqlB); # Executa la busqueda y obtiene el registro a actualizar.
                                $radiNumero = $rs->fields["SGD_SRD_CODIGO"];
                                if ($radiNumero == '') {
                                    echo "<script>alert('El código no existe, por favor verifique la información e intente de nuevo');</script>";
                                } else {
                        ...
                        if (isset($_POST['modi_subserie'])) {
                            if ($codserie != 0 && $tsub != 0 && $detasub != '') {
                                $isqlB = "select * from sgd_sbrd_subserierd where sgd_srd_codigo = $codserie and sgd_sbrd_descrip = '$detasub' and sgd_sbrd_codigo != $tsub";
                                $rs = $db->query($isqlB); # Executa la busqueda y obtiene el registro a actualizar.
                                $radiNumero = $rs->fields["SGD_SRD_CODIGO"];
                                if ($radiNumero != '') {
                                    echo "<script>alert('La subserie ya existe, por favor verifique la información e intente de nuevo');</script>";
                                } else {
								...
								";
                                    ...
                                    $soporte = '';
                                if($rsUp){
                                    echo "<script>alert('Se modifico la subserie correctamente');</script>";
                                }else{
                                    echo "<script>alert('No se pudo realizar la actualización verifique');</script>";
                                }		
                                    ?>
                                    <script language="javascript">
                                        document.adm_subserie.elements['detasub'].value = '';
...
include_once "$dir_raiz/trd/lista_subseries.php";
?>
</form>
</span>
<br><br>
