...
/* ---------------------------------------------------------+
  ...
  $codserieI = $_POST['codserieI'];
  $detaserie = $_POST['detaserie'];
  $fecha_busq = $_POST['fecha_busq'];
  $fecha_busq2 = $_POST['fecha_busq2'];
  $insertar_serie = $_POST['insertar_serie'];
      ...
      <head>
          ...
          <script>
              ...
              function val_datos()
              {
                  ...
                  if (isWhitespace(document.getElementById('fecha_busq').value))
                  {
                      err_msg = err_msg + 'Digite Fecha Inicial.\n';
                      bandera = false;
                  }
                  if (isWhitespace(document.getElementById('fecha_busq2').value))
                  {
                      err_msg = err_msg + 'Digite Fecha Final.\n';
                      bandera = false;
                  }
  ...
  //Busca series que cumplen con el detalle
                  ...
                  if (isset($insertar_serie) && $codserieI != 0 && $detaserie != '') {
                      ...
                      $codSeries = "select sgd_srd_codigo from sgd_srd_seriesrd order by sgd_srd_codigo desc";
                      $rSeries = $db->query($codSeries); # Executa la busqueda y obtiene el registro a actualizar.        
                      $codserieI = $rSeries->fields["SGD_SRD_CODIGO"] + 1;
                      ...
                      if ($radiNumero != '') {
                          ...
                          if ($radiNumero != '') {
                              ...
                              //  $codSeries = "select sgd_srd_codigo from sgd_srd_seriesrd order by sgd_srd_codigo desc limit 1";
                              //  $rSeries = $db->query($codSeries); # Executa la busqueda y obtiene el registro a actualizar.	
                              //  $codserieI = $rSeries->fields["SGD_SRD_CODIGO"]+1;
              ...
              document.adm_serie.detaserie.value = '';
                                  ...
                                  <?php
                                  if (isset($rsIN)) {
                                      $mensaje_err = "<HR><center><B><FONT COLOR=RED>SE CREO CORRECTAMENTE LA SERIE < $codserieI > $detaserie </FONT></B></center><HR>";
                                  }
                              }
                          }
                      }
                      if (isset($actua_serie) && $codserieI != 0 && $detaserie != '') {
                          ...
                          if ($radiNumero == '') {
                              ...
                              $isqlB = "select * from sgd_srd_seriesrd where sgd_srd_descrip = '$detaserie' and sgd_srd_codigo != $codserieI";
                              $rs = $db->query($isqlB); # Executa la busqueda y obtiene el registro a actualizar.
                              $radiNumero = $rs->fields["SGD_SRD_CODIGO"];
                              if ($radiNumero != '') {
                                  ...
                                  SGD_SRD_FECHFIN =$sqlFechaH where sgd_srd_codigo = $codserieI";
                                  $rsUp = $db->query($isqlUp);
                                  $codserieI = 0;
                                  $detaserie = '';
                                  $mensaje_err = "<HR><center><B><FONT COLOR=RED>SE MODIFIC&Oacute; LA SERIE " . $detaserie . "</FONT></B></center><HR>";
                                  ?>
              <script language="javascript">
              document.adm_serie.codserieI.value = '';
              document.adm_serie.detaserie.value = '';
                      ...
                      echo $mensaje_err;
                      include_once "$dir_raiz/trd/lista_series.php";
                      ?>
          </form>
          <p>
              <?php //$mensaje_err  ?>
