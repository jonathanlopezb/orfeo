...
//            $anuId = 1;
            ...
            $sql = "insert into sgd_anu_anulados (RADI_NUME_RADI, SGD_EANU_CODI, SGD_ANU_SOL_FECH, DEPE_CODI  , USUA_DOC, SGD_ANU_DESC ,USUA_CODI)
				        values ('$noRadicado'   ,1 , $systemDate ,
						'$dependencia',$usuadoc , '$comentario',$codUsuario)";
...
# Codigo de prueba para UPDATE 
        ...
        $sql = "SELECT USUA_ANU_ACTA FROM sgd_anu_anulados WHERE USUA_ANU_ACTA = " . $actaNo. $where_depe . $where_TipoRadicado;

        # Selecciona el registro a actualizar
        $rs = $this->db->conn->Execute($sql); # Executa la busqueda y obtiene el registro a actualizar.

        if ($rs->RowCount() >= 1)
            die("<HR><center><B><FONT COLOR=RED>EL ACTA No < $actaNo > YA EXISTE. <BR>  VERIFIQUE LA INFORMACION E INTENTE DE NUEVO</FONT></B></center><HR>");

        // echo "-->" . $rs->RowCount();
        if ($radicados) {
            foreach ($radicados as $noRadicado) {

                $sql = "SELECT SGD_EANU_CODIGO FROM radicado WHERE radi_nume_radi = '" . $noRadicado . "'";
                ...
                $this->db->conn->Execute($updateSQL); # Actualiza el registro en la base de datos
                # Insertamos en la tabla sgd_anu_anulados los registros que entran en solicitud.
                $sql = "update sgd_anu_anulados   set SGD_EANU_CODI=2, DEPE_CODI_ANU= '$dependencia',
				USUA_DOC_ANU=$usuadoc,USUA_CODI_ANU=$codUsuario,USUA_ANU_ACTA=$actaNo,
				SGD_TRAD_CODIGO=$tipoRadicado,SGD_ANU_PATH_ACTA='$pathActa'
			where radi_nume_radi = '$noRadicado'";
