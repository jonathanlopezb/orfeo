...
class CONTROL_ORFEO {
    ...
    function datos_expediente($radicado, $num_expediente) {
        $estado_expediente = 0;
        $query = "select max(SGD_EXP_CARPETA) tt from sgd_exp_expediente WHERE SGD_EXP_NUMERO='$num_expediente' group by SGD_EXP_NUMERO ";
        $rs = $this->cursor->query($query);
        if (!$rs) {
    ...
    function grabar_usuario($no_documento, $nombre_us1, $direccion_us1, $prim_apell_us1, $seg_apell_us1, $telefono_us1, $mail_nus1, $codep_us, $muni_us) {
        ...
        if (!$rs) {
            $this->cursor->conn->RollbackTrans();
            die("<span class='etextomenu'>No se ha podido insertar la información del nuevo usuario ($query)");
        }
    ...
    function tabla_sql($query) {
        ...
        if ($jh_db) {
            ...
            echo "<center><p><b>". "N&uacute;mero" ." de Registros " . $numreginf . "</b></center>";
        }
        global $ADODB_FETCH_MODE;
        $ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
        $jh_db = $this->cursor->conn->Execute($query);
        $tabla_html = "<table border=1 width=95%>";
        echo "<table class='borde_tab' align='center' border='2' width=95%>";
        if ($jh_db) {
